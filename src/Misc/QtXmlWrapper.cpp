	...
	d( new XmlData )
{
	...
	d->m_node.setAttribute( "version-major", QString::number( version.get_major() ) );
	d->m_node.setAttribute( "version-minor", QString::number( version.get_minor() ) );
	d->m_node.setAttribute( "version-revision", QString::number( version.get_revision() ) );
...
int QtXmlWrapper::saveXMLfile(const std::string &filename, int compression) const
{
...
int QtXmlWrapper::loadXMLfile(const std::string &filename)
{
    ...
    m_fileversion.set_major(root.attribute( "version-major" ).toInt());
    m_fileversion.set_minor(root.attribute( "version-major" ).toInt());
    m_fileversion.set_revision(root.attribute( "version-major" ).toInt());

    if(verbose)
	std::cout << "loadXMLfile() version: " << m_fileversion << std::endl;
...
void QtXmlWrapper::add(const XmlNode &node_)
{
	QDomElement element = d->m_doc.createElement( node_.name.c_str() );
	d->m_node.appendChild( element );
	for(auto attr:node_.attrs)
		element.setAttribute(attr.name.c_str(), attr.value.c_str());
}

std::vector<XmlNode> QtXmlWrapper::getBranch(void) const
{
	std::vector<XmlNode> res;
	QDomNode node = d->m_node.firstChild();
	while(!node.isNull()) {
		if(node.isElement()) {
			QDomElement elm = node.toElement();
			XmlNode n(node.nodeName().toStdString());
			QDomNamedNodeMap attrs = elm.attributes();
			for(int i = 0; i < attrs.count(); ++i) {
				const QDomAttr &attr = attrs.item(i).toAttr();
				n[attr.name().toStdString()] = attr.value().toStdString();
			}
			res.push_back(n);
		}
